...
public class AddressBook {
    ...
    private static final String COMMAND_CHANGE_DESC = "Changes details to person in address book.";
    private static final String COMMAND_CHANGE_PARAMETERS = "1/2/3 as to replace name/phone/email "
                                                            + "name to replace " + "replacement";
    private static final String COMMAND_CHANGE_EXAMPLE = COMMAND_CHANGE_WORD + " 1 John Jon";
    ...
    private static String executeCommand(String userInputString) {
        ...
        case COMMAND_CHANGE_WORD:
            return executeChangeDetails(commandArgs);
    ...
    private static String executeChangeDetails (String commandArgs){
        final Set<String> keywords = extractKeywordsFromFindPersonArgs(commandArgs);
        String WordList[] = commandArgs.split(" ");
        String replaceWord = WordList[2];
        int Type = Integer.valueOf(WordList[0]);
        final ArrayList<HashMap<String,String>> personsFound = getPersonsWithNameContainingAnyKeyword(keywords);
        EditPersonDetails(personsFound,Type,replaceWord);
        return getMessageForFormattedPersonData(personsFound.get(0));
    }
    ...
    */
     ...
     * Edit peron's detail
     */
    private static void EditPersonDetails(ArrayList<HashMap<String, String>> Person, int Type, String Details){

        //if()  Find if theres such a person
        switch (Type){
            case 1:
                    Person.get(0).put(PERSON_DATA_INDEX_NAME, Details);
                break;

            case 2:
                    Person.get(0).put(PERSON_DATA_INDEX_PHONE, Details);

                break;

            case 3:
                    Person.get(0).put(PERSON_DATA_INDEX_EMAIL,Details);
                break;
        }
    }
